...
## Generating samples (Dataset 2)

```{r}
N = 600
sigma <- matrix(data=c(2,-1.7,-1.7,2), nrow=2)

X_group1 <-  mvrnorm(n = N/4, mu = c(6,-6), Sigma = sigma, empirical = FALSE)
X_df1 <- data.frame(axis1 = X_group1[,1],axis2 = X_group1[,2],group = rep("Foreground Group 1",N/4))

X_group2 <- mvrnorm(n = N/4, mu = c(-6,6), Sigma = sigma, empirical = FALSE)
...
Y_df <- data.frame(axis1 = Y[,1],axis2 = Y[,2],group = rep("Background Group",N/2))
